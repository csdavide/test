...
public class AssociationServiceBridge extends AbstractServiceBridge {
    ...
    public void renameContent(Node source, String nameValue, String propertyPrefixedName,
        ...
        call(context, () -> {
            try {
                ...
                rename.setPropertyName(propertyPrefixedName);
                rename.setName(nameValue);
                rename.setRenameMode(onlyPrimaryAssociation ? RenameStatement.RenameMode.FIRST_PARENT : RenameStatement.RenameMode.ALL_PARENTS);
                nodeService.renameNode(new Vertex(VertexType.UUID, source.getUid()), rename);
                return null;
    ...
    public void unLinkContent(Node source, Node destination, Association association, MtomOperationContext context)
        ...
        call(context, () -> {
            ...
            var link = asLink(source.getUid(), association, false);
            try {
                var r = associationMapper.relationship(uuid, link);
                associationService.unlinkNode(r.vertex(), r.parentLink());
    ...
    public void moveNodeAssociation(Node source, Node parent, Association newAssociation, MtomOperationContext context)
        ...
        call(context, () -> {
            var destination = new ParentLink();
            ...
            try {
                nodeService.moveNode(new Vertex(VertexType.UUID, source.getUid()), destination);
                return null;
            } catch (ConflictException e) {
                throw new InsertException(e.getMessage(), e);
            }
