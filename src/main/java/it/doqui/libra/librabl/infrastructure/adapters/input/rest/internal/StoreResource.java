package it.doqui.libra.librabl.infrastructure.adapters.input.rest.internal;
...
@IfBuildProperty(name = "libra.module.old-files.enabled", stringValue = "true")
@Path("/internal/files")
@Slf4j
public class StoreResource {
    ...
    public Response getFileData(@QueryParam("contentUrl") String contentUrl, @Context HttpHeaders headers) throws IOException {
        ...
        var cxt = sessionContext.getUserContext();
        if (cxt != null) {
    ...
    public Response writeFileData(@PathParam("contentUrl") String contentUrl, @Context HttpHeaders headers, @RequestBody InputStream is) throws IOException {
        ...
        log.info("Storing file to {} (user {})", contentUrl, Optional.ofNullable(sessionContext.getUserContext()).map(UserContext::getAuthorityRef).map(Object::toString).orElse("anonymous"));
    ...
    @Consumes(MediaType.WILDCARD)
    @Operation(hidden = true)
    public Response writeFileData(@PathParam("contentUrl") String contentUrl, @Context HttpHeaders headers) {
        verifySharedSecret(headers);

        log.info("Deleting url {} (user {})", contentUrl, Optional.ofNullable(sessionContext.getUserContext()).map(UserContext::getAuthorityRef).map(Object::toString).orElse("anonymous"));
        try {
