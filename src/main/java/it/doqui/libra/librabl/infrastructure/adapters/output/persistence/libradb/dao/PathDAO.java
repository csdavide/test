...
public class PathDAO extends AbstractDAO {
    ...
    public Optional<NodeInfoItem> findNodeIdWherePath(String path) {
        ...
        var tenant = sessionContext.getUserContext().getTenantRef().toString();
        var p = Strings.CS.endsWith(path, "/") ? path : path + "/";
        return call(conn -> {
            final var sql = """
                select p.node_id,n.uuid,n.type_name,n.code\s
                from ecm_paths p\s
            ...
            try (var stmt = conn.prepareStatement(sql)) {
                ...
                try (var rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        var n = NodeInfoItem.builder()
                            .id(rs.getLong("node_id"))
                            .tenant(tenant)
    ...
    public Map<String, List<String>> pathOfUUIDs(Collection<String> uuids) {
        ...
        return call(conn -> {
            final var sql = """
                ...
                order by n.id, p.is_hard desc, p.id
                """;
            try (var stmt = conn.prepareStatement(sql)) {
    ...
    public Optional<String> findNodePathByNodeId(long nodeId, Optional<Long> parentId) {
        return call(conn -> {
            final var sql = """
                select p.node_path\s
                from ecm_nodes n\s
                join ecm_paths p on (p.node_id = n.id)\s
                where n.tenant = ? and p.node_id = ?
                and case when ? then p.parent_id = ? else true end
            """;
            try (var stmt = conn.prepareStatement(sql)) {
                stmt.setString(1, sessionContext.getTenant());
                stmt.setLong(2, nodeId);
                stmt.setBoolean(3, parentId.isPresent());
                stmt.setLong(4, parentId.orElse(0L));
                try (var rs = stmt.executeQuery()) {
                    return rs.next() ? Optional.of(rs.getString("node_path")) : Optional.empty();
                }
            } catch (SQLException e) {
