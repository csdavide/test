package it.doqui.libra.librabl.infrastructure.adapters.output.solr.provisioning;

import it.doqui.libra.librabl.infrastructure.adapters.output.schema.TenantSchema;
...
import it.doqui.libra.librabl.domain.model.schema.IndexableProperty;
...
public class SolrManager extends AbstractSolrController {
    ...
    @ConfigProperty(name = "solr.zipConfigSetPath", defaultValue = "./solr-configset.zip")
    String zipConfigSetPath;
    ...
    private void uploadConfigSet(String collectionName, boolean overwrite) throws IOException, SolrServerException {
        ...
        if (configSetPath.endsWith(".zip")) {
            ...
            configSetPathFile = new File(zipConfigSetPath);
    ...
    public void syncTenant(TenantRef tenantRef, TenantSchema schema, boolean coreFields) {
        ...
        try {
            ...
            final Set<String> processedFieldNames = new HashSet<>();
            for (CustomModelSchema model : schema.getNamespaces().values()) {
                ...
                for (var pd : model.getProperties()) {
                    ...
                    if (f == null) {
                        ...
                        if (!fakeIndexModeEnabled && type != null) {
                        ...
                        if (type != null) {
                            ...
                            if (updateRequired && !fakeIndexModeEnabled) {
            ...
            fieldMap.values().stream().map(SolrField::getName).filter(f -> f.startsWith("@")).filter(f -> !processedFieldNames.contains(f)).forEach(f -> {
                log.info("Removing field {} from solr collection {}", f, collectionName);
                try {
                    deleteField(collectionName, f);
                } catch (Exception e) {
                    log.warn("Could not remove field {} from solr collection {}", f, collectionName);
                }
            });

            dynamicFieldMap.values().stream().map(SolrField::getName).filter(f -> f.startsWith("@")).filter(f -> !StringUtils.equals(f, "@*")).filter(f -> !processedFieldNames.contains(f)).forEach(f -> {
                log.info("Removing dynamic field {} from solr collection {}", f, collectionName);
                try {
                    deleteDynamicField(collectionName, f);
                } catch (Exception e) {
                    log.warn("Could not remove dynamic field {} from solr collection {}", f, collectionName);
                }
            });
            log.info("Tenant {} fully synchronized with solr collection {}", tenantRef, collectionName);
        } catch (SolrServerException | IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private String mapAsSolrType(CustomModelSchema model, IndexableProperty pd) {
        ...
        if (Strings.CS.equals(qtype.getNamespaceURI(), "d")) {
    ...
    private void deleteDynamicField(String collectionName, String name) throws SolrServerException, IOException {
        try {
            final var request = new SchemaRequest.DeleteDynamicField(name);
            final var response = request.process(client, collectionName);
            log.debug("Solr deleteDynamicField '{}' returned {}", name, response);
        } catch (BaseHttpSolrClient.RemoteExecutionException e) {
            if (e.code() == 400) {
                throw new BadRequestException(e.getMessage());
            }
    ...
    private void deleteField(String collectionName, String name) throws SolrServerException, IOException {
        try {
            final SolrRequest<SchemaResponse.UpdateResponse> request = new SchemaRequest.DeleteField(name);
            final SchemaResponse.UpdateResponse response =  request.process(client, collectionName);
            log.debug("Solr deleteField '{}' returned {}", name, response);
            if (response.getStatus() != 0) {
                String msg = String.format("Solr returned status %d deleting field to schema", response.getStatus());
                throw new WebException(response.getStatus(), msg);
            }
        } catch (BaseHttpSolrClient.RemoteExecutionException e) {
            if (e.code() == 400) {
