...
public class MultipleContentResource extends AbstractContentResource {
    ...
    public Response updateNodeContent(
        ...
        return call(() -> {
            contentUseCase.setNodeContent(uuid, makeContentStream(contentPropertyName, Optional.ofNullable(StringUtils.stripToNull(customContentType)).orElse(contentType), contentDisposition, body), currentFilename);
    ...
    public Response addNodeContent(
        ...
        return call(() -> {
            contentUseCase.addNodeContent(uuid, makeContentStream(contentPropertyName, Optional.ofNullable(StringUtils.stripToNull(customContentType)).orElse(contentType), contentDisposition, body));
            return Response.noContent().build();
        });
    }
    ...
    public Response removeNodeContent(
        ...
        @QueryParam("fileName") String fileName) {
        return call(() -> {
            contentUseCase.removeNodeContent(uuid, contentPropertyName, fileName);
            return Response.noContent().build();
        });
    }
    ...
    @POST
    @Path("/{contentPropertyName}/seals")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(operationId = "sealNodeContent", summary = "Seals a node content")
    @Traceable(traceAllParameters = true, category = TraceCategory.CREATE)
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "A sealing response is returned", content = @Content(schema = @Schema(implementation = DocumentOperationResponse.class))),
        @APIResponse(responseCode = "403", description = "Forbidden"),
        @APIResponse(responseCode = "404", description = "The specified node is not found"),
        @APIResponse(responseCode = "412", description = "The specified node content is not found"),
        @APIResponse(responseCode = "500", description = "Unexpected error")
    ...
    @POST
    @Path("/{contentPropertyName}/signs")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Operation(operationId = "signNodeContent", summary = "Signs a node content")
    @Traceable(traceAllParameters = true, category = TraceCategory.CREATE)
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "A signing response is returned", content = @Content(schema = @Schema(implementation = DocumentOperationResponse.class))),
        ...
        @APIResponse(responseCode = "403", description = "Forbidden"),
        @APIResponse(responseCode = "404", description = "The specified node is not found"),
        @APIResponse(responseCode = "412", description = "The specified node content is not found or wrong sign type requested"),
        @APIResponse(responseCode = "500", description = "Unexpected error")
    })
    public Response signNodeContent(
        @PathParam("uuid") String uuid,
        @PathParam("contentPropertyName") String contentPropertyName,
        @QueryParam("fileName") String fileName,
        @RequestBody(
            description = "Params requested for signing",
            content = @Content(schema = @Schema(implementation = SignRequest.class))
        ) @TraceParam(ignore = true) SignRequest signRequest
    ) {
        return signDocument(uuid, contentPropertyName, fileName, signRequest);
    }
    ...
    @Traceable(traceAllParameters = true, category = TraceCategory.READ)
    @APIResponses(value = {
        @APIResponse(responseCode = "200", description = "A certificate report is returned", content = @Content(schema = @Schema(implementation = DocumentOperationResponse.class))),
        @APIResponse(responseCode = "403", description = "Forbidden"),
        @APIResponse(responseCode = "404", description = "The specified node is not found"),
        @APIResponse(responseCode = "412", description = "The specified node content is not found"),
        @APIResponse(responseCode = "500", description = "Unexpected error")
