...
import it.doqui.libra.librabl.infrastructure.adapters.input.rest.dto.mimetypes.MimeTypeItem;
import it.doqui.libra.librabl.infrastructure.adapters.input.rest.dto.mimetypes.MimeTypeRequest;
...
public class MimeTypeResource extends AbstractResource {
    ...
    public Response addMimeType(@RequestBody MimeTypeRequest item) {
        return call(() -> {
            mimeTypeUseCase.addAll(List.of(modelMapper.map(item, MimeTypeMapping.class)));
    ...
    public Response getMimeType(@PathParam("id") long id) {
        return call(() -> mimeTypeUseCase.getById(id)
    ...
    public Response deleteMimeTypes(
        ...
        return call(() -> {
            mimeTypeUseCase.delete(ObjectUtils.map(flat(ids), Long::parseLong));
            return Response.noContent().build();
        });
    }
    ...
    public Response addMimeTypes(@RequestBody List<MimeTypeRequest> items) {
        return call(() -> {
            mimeTypeUseCase.addAll(items.stream().map(item -> modelMapper.map(item, MimeTypeMapping.class)).toList());
            return Response.noContent().build();
        });
    }
    ...
    public Response replaceMimeTypes(@RequestBody List<MimeTypeRequest> items) {
        return call(() -> {
            mimeTypeUseCase.replaceAll(items.stream().map(item -> modelMapper.map(item, MimeTypeMapping.class)).toList());
            return Response.noContent().build();
        });
    }
    ...
    public Response listMimetypes(
        ...
        return call(() -> {
            ...
            var mimetypes = mimeTypeUseCase.list(modelMapper.map(mt, MimeTypeMapping.class), includeStarExtensions);
            return Response.ok(mimetypes.stream().map(m -> modelMapper.map(m, MimeTypeItem.class))).build();
        });
    }
}
