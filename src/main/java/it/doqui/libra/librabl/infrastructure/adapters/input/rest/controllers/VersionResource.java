...
public class VersionResource extends AbstractResource {
    ...
    public Response listNodeVersions(
        ...
        return call(() -> Response.ok(versioningUseCase.listNodeVersions(new Vertex(VertexType.UUID, uuid), flat(tags))).build());
    }
    ...
    public Response createNodeVersion(
        ...
        return call(() -> versioningUseCase.createNodeVersion(new Vertex(VertexType.UUID, uuid), tag)
            .map(item -> {
    ...
    public Response tagNodeVersion(
        ...
        return call(() -> {
            versioningUseCase.alterTagVersion(new Vertex(VertexType.UUID, uuid), version, tag);
    ...
    public Response getNodeVersion(
        ...
        @QueryParam("locale")
        String locale) {
        return call(() -> versioningUseCase
            .getNodeVersion(new Vertex(VertexType.UUID, uuid), version, MapOption.valueOf(options), I18NUtils.parseLocale(locale))
    ...
    public Response retrieveDefaultNodeVersionContent(
        ...
        try {
            return getNodeContent(new Vertex(VertexType.UUID, uuid), version, null, inline, fileName);
    ...
    public Response retrieveNodeVersionContent(
        ...
        @QueryParam("inline") @DefaultValue("false") boolean inline) {
        try {
            return getNodeContent(new Vertex(VertexType.UUID, uuid), version, contentPropertyName, inline, fileName);
        } catch (PreconditionFailedException e) {
            throw new NotFoundException(e.getMessage());
        }
    }

    private Response getNodeContent(Vertex vertex, int version, String contentPropertyName, boolean inline, String fileName) {
        return call(() -> {
            NodeAttachment a = versioningUseCase.getVersionedContent(vertex, version, contentPropertyName, fileName);
