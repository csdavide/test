...
public class ContentServiceBridge extends AbstractServiceBridge {
    ...
    public Attachment downloadMethod(String uid, String usr, String pwd, String repo, String prefixedName) {
        ...
        return call(context, () -> {
            NodeAttachment a = contentUseCase.getNodeContent(new ContentRef().setUuid(uid).setContentPropertyName(prefixedName));
            if (!Strings.CS.equals(a.getContentProperty().getName(), prefixedName)) {
    ...
    public byte[] retrieveContentData(Node node, Content content, MtomOperationContext context)
        ...
        return call(context, () -> {
            NodeAttachment a = contentUseCase.getNodeContent(new ContentRef().setUuid(node.getUid()).setContentPropertyName(cname));
    ...
    public File[] massiveRetrieveFiles(Node[] nodes, Content[] contents, MtomOperationContext context)
        ...
        return call(context, () -> {
            Map<String,NodeAttachment> attachmentMap = contentUseCase.getNodeContents(inputs.values(), MASSIVE_MAX_RETRIEVE_SIZE);
            ...
            for (Map.Entry<String,ContentRequest> entry : inputs.entrySet()) {
                ...
                if (entry.getValue() == null || !Strings.CS.equals(a.getContentProperty().getName(), cname)) {
                    throw new InvalidParameterException(
    ...
    public String directUploadMethod(@TraceParam(ignore = true) Attachment myFile, String usr, String pwd, String repo, String uuid, String prefixedName) throws SystemException {
        try {
            ...
            validate(() -> Objects.requireNonNull(myFile, "Attachment must not be null"));
            ...
            return call(context, () -> {
                ...
                contentUseCase.setNodeContent(uuid, cs, null);
                return uuid;
            });
        } catch (RuntimeException e) {
            try (var is = myFile.attachmentDataHandler.getInputStream()) {
                log.debug("Ignored attachment: {} bytes discarded", IOUtils.discard(is));
    ...
    public String uploadMethod(@TraceParam(ignore = true) Attachment myFile, String usr, String pwd, String repo, String parent) throws SystemException {
        try {
            MtomOperationContext context = new MtomOperationContext();
            context.setUsername(usr);
            context.setPassword(pwd);
    ...
    public void updateContentData(Node node, Content content, MtomOperationContext context)
        ...
        call(context, () -> {
            ...
            contentUseCase.setNodeContent(node.getUid(), cs, null);
            return null;
        });
    }
    ...
    public FileInfo getFileInfo(Node node, NodeInfo nodeInfo, MtomOperationContext context)
        ...
        return call(context, () -> {
            var a  = contentUseCase.getNodeContent(new ContentRef().setUuid(node.getUid()).setContentPropertyName(nodeInfo.getPrefixedName()));
    ...
    public FileFormatInfo[] getFileFormatInfoFileInfo(FileInfo fileInfo, MtomOperationContext context)
        ...
        return call(context, () -> transactionManagerPort.doOnTemp(() -> transactionManagerPort.requireNew(() -> {
            ...
            var ephemeralContent = temporaryUseCase.createEphemeralNode(descriptor, new ByteArrayInputStream(fileInfo.getContents()), null);
            ...
            try {
                ...
                if (fileInfo.isStore()) {
                    for (var fi : result) {
                        ...
                        temporaryUseCase.unephemeralize(ephemeralContent.getUuid());
                    }
                }
