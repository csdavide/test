...
import it.doqui.libra.librabl.infrastructure.adapters.output.persistence.libradb.dao.deleters.AbstractDeleterDAO;
import it.doqui.libra.librabl.infrastructure.adapters.output.persistence.libradb.dao.deleters.InsertModeDeleterDAO;
...
import static it.doqui.libra.librabl.domain.model.graph.IndexingFlags.FULL_FLAG_MASK;
import static it.doqui.libra.librabl.domain.model.graph.IndexingFlags.formatAsBinary;

@ApplicationScoped
@Slf4j
public class ArchiveDAO extends AbstractDeleterDAO {

    @Inject
    ObjectMapper objectMapper;

    @Inject
    InsertModeDeleterDAO associationDeleterDAO;

    public Optional<ArchivedNode> getNode(Vertex vertex) {
        if (vertex == null) {
            throw new IllegalArgumentException("Vertex cannot be null");
        }

        var q = switch (vertex.getType()) {
            case ID -> "n.id = ?";
            case UUID -> "n.tenant = ? and n.uuid = ?";
            default -> throw new IllegalArgumentException("Invalid vertex type: " + vertex.getType());
        };

        return getNodeWhere(q, (stmt, c) -> {
            try {
                if (vertex.getType() == VertexType.ID) {
                    stmt.setLong(++c, Long.parseLong(vertex.getValue()));
                } else {
                    stmt.setString(++c, sessionContext.getUserContext().getTenantRef().toString());
                    stmt.setString(++c, vertex.getValue());
                }
    ...
    private Optional<ArchivedNode> getNodeWhere(String where, BiFunction<PreparedStatement, Integer, Integer> setParamsFunc) {
        return call(conn -> {
            var sql = "select n.id,n.tenant,n.uuid,n.type_name,n.data,n.updated_at,n.tx";

            if (!sessionContext.getUserContext().isAdmin()) {
                sql += """
                ,case when n.sg_id is null or n.sg_id in (\s
                       ...
                       ) or n.data->'properties'->>'sys:archivedBy' = ?\s
                         or case when n.data->'properties'->>'cm:owner' is not null\s
                            then n.data->'properties'->>'cm:owner' = ?\s
                            else n.data->'properties'->>'cm:creator' = ? end then true else false end has_permissions
                """;
            } else {
                sql += ",true has_permissions";
            }
            sql += " from ecm_archived_nodes n where\s";
            ...
            try (var stmt = conn.prepareStatement(sql)) {
                int c = 0;
                if (!sessionContext.getUserContext().isAdmin()) {
                    String username = sessionContext.getUserContext().getAuthority();
                ...
                try (var rs = stmt.executeQuery()) {
                    ...
                    if (rs.getBoolean("has_permissions")) {
                        var n = readArchivedNode(rs, true);
                        return Optional.of(n);
                    } else {
                        return Optional.of(new ArchivedNode());
                    }
                }
    ...
    public Map<Long, List<ArchivedAssociation>> findParentAssociations(Collection<Long> nodeIds, boolean hardOnly) {
        ...
        return call(conn -> {
            var sql = """
                ...
                where a.child_id = any(?) and case when ? then a.is_hard else true end\s
                order by a.id
                """;

            try (var stmt = conn.prepareStatement(sql)) {
                stmt.setArray(1, conn.createArrayOf("BIGINT", nodeIds.toArray(new Long[0])));
                stmt.setBoolean(2, hardOnly);
                try (var rs = stmt.executeQuery()) {
                    var result = new HashMap<Long, List<ArchivedAssociation>>();
                    while (rs.next()) {
    ...
    public Optional<ParentLink> findRestorableParentAssociation(long nodeId) {
        return call(conn -> {
            var sql = """
            ...
            try (var stmt = conn.prepareStatement(sql)) {
                ...
                try (var rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        link.setType(rs.getString("type_name"));
                        link.setName(rs.getString("name"));
                        link.setParent(new Vertex(VertexType.UUID, rs.getString("uuid")));
    ...
    private int purgeUnreachableNodes(Connection conn, long oldTx, long newTx, long rootId, boolean isClosureRoot, boolean wipeable) throws SQLException {
        ...
        if (isClosureRoot) {
                ...
                """))
            {
                ...
                if (n > limit) {
                    throw new LimitExceededException("Purge limit exceeded: " + limit);
                }
            ...
            insertRemovedNodesWhereTx(conn, oldTx, newTx, rootId, wipeable, limit);
        }
        ...
        associationDeleterDAO.purgeNodeAccessories(conn, new TransactionalNodesItem(null, newTx));
        cleanSecurityGroups(conn, newTx);
        return nodes;
    }

    private void insertRemovedNodesWhereTx(Connection conn, long oldTx, long newTx, long rootId, boolean wipeable, int limit) throws SQLException {
        try (var stmt = conn.prepareStatement("""
    ...
    private void restoreEmbeddedAssociations(Connection conn, String field, boolean isChild, long oldTx, Long newTx) throws SQLException {
        var sql = String.format("""
                ...
                """,
            Strings.CS.contains(field, "Source") || Strings.CS.contains(field, "Target") ? "null" : "z.hard",
