...
import it.doqui.libra.librabl.domain.model.schema.ModelNamespace;
...
//        }
    ...
    private void validateProperties(CustomModelSchema model, Map<String,CustomModelSchema> importMap) {
        ...
        for (var pd : model.getDynamicProperties()) {
            ...
            if (!pd.isPredefined() && !Strings.CS.endsWith(pname.getLocalPart(), "*") || StringUtils.countMatches(pname.getLocalPart(), '*') != 1) {
    ...
    private void validateTypedInterfaces(CustomModelSchema model, Map<String,CustomModelSchema> importMap) {
        ...
        for (var t : model.getTypes()) {
            validateTypedInterface("type", processedSet, t, nsSet, importMap);
        }

        for (var t : model.getAspects()) {
            validateTypedInterface("aspect", processedSet, t, nsSet, importMap);
        }
    }

    private void validateTypedInterface(String kind, Set<String> processedSet, TypedInterfaceDescriptor t, Set<String> nsSet, Map<String,CustomModelSchema> importMap) {
        ...
        if (StringUtils.isBlank(t.getParent())) {
            if (Strings.CS.equals(kind, "type") && !Strings.CS.equals(t.getName(), "sys:base")) {
