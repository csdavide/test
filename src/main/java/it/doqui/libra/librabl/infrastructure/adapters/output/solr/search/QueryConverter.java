...
public class QueryConverter {
    ...
    private static final Pattern minIntervalElementPattern = Pattern.compile("( )*(MIN|min|\\*|" + DATE_REGEX + ")( )*");
    private static final Pattern maxIntervalElementPattern = Pattern.compile("( )*(MAX|max|\\*|" + DATE_REGEX + ")( )*");
    ...
    private static final Pattern rangePattern = Pattern.compile("\\[" + minIntervalElementPattern + "(TO|to)" + maxIntervalElementPattern + "]|" + QUOTE_DATE_REGEX);
    ...
    public QueryConverter(ModelSchema schema, String additionalTokenizedSuffix) {
        this.schema = schema;
...
//    }
    ...
    private String convertQuotedFields(ModelSchema schema, final String q) {
        ...
        while (matcher.find()) {
            ...
            if (Strings.CS.startsWith(s, "workspace:")) {
                var p = s.lastIndexOf('/');
                if (p > 0) {
            ...
            if (matcher.start() >= p && matcher.end() < q.length()) {
                ...
                if (quoted) {
                    ...
                    if (quote > matcher.end()) {
                        ...
                        if (!Strings.CS.startsWith(quotedString, "@") && schema.getNamespaceSchema(quotedString) != null) {
                            ...
                            case "d:date", "d.datetime":
                                if (StringUtils.isNumeric(elem)) {
                                    try {
                                        ...
                                        log.error("Unable to parse number '{}': {}", elem, e.getMessage());
                                    }
                                }
                                break;
                        }
