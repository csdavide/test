...
import it.doqui.libra.librabl.infrastructure.adapters.input.rest.controllers.AbstractResource;
...
public abstract class AbstractBridgeResource extends AbstractResource {
    ...
    public Response call(String tenant, Callable<Response> task) {
        return super.call(() -> {
            if (!Strings.CI.equals(tenant, sessionContext.getTenant())) {
    ...
    protected FormValue getInputPart(String param, MultipartFormDataInput input, boolean required) throws BadRequestException {
        var formDataMap = input.getValues();
        ...
        if (inputParts != null && !inputParts.isEmpty()) {
            ...
            return inputParts.stream().findFirst().orElse(null);
        } else if (required) {
    ...
    protected String getStringFromMultipart(String param, MultipartFormDataInput input, boolean required) throws BadRequestException {
        return Optional.ofNullable(getInputPart(param, input, required)).map(FormValue::getValue).orElse(null);
    }
    ...
    protected <T> T getObjectFromMultipart(String param, MultipartFormDataInput input, boolean required, Class<T> targetClass) {
        ...
        try {
            return objectMapper.readValue(part.getValue(), targetClass);
        } catch (IOException e) {
    ...
    protected byte[] getByteArraysFromMultipart(String param, MultipartFormDataInput input, boolean required) throws BadRequestException {
        return Optional.ofNullable(getInputPart(param, input, required))
            .map(p -> Optional.ofNullable(p.getFileItem())
                .map(f -> {
                    try {
                        return IOUtils.readFully(f.getInputStream());
                    } catch (IOException e) {
                        throw new SystemException(e);
                    }
                })
