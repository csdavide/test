package it.doqui.libra.librabl.infrastructure.adapters.input.index.cxf.mappers;
...
import it.doqui.libra.librabl.application.model.document.PDFAnalyzer;
import it.doqui.libra.librabl.application.model.graph.InputIdentifiedNodeRequest;
...
public class ContentMapper {
    ...
    public FileReport asFileReport(FileFormatDescriptor ff) {
        ...
        result.setPasswordProtected(pwdProtectedAnalyzers != null && !pwdProtectedAnalyzers.isEmpty());
        result.setDamaged(damagedAnalyzers != null && (damagedAnalyzers.contains(PDFAnalyzer.OPENPDF) && damagedAnalyzers.contains(PDFAnalyzer.PDFBOX) || damagedAnalyzers.contains(PDFAnalyzer.TIKA)));
    ...
    private boolean isPdfAnalysisValid(FileFormatDescriptor ff) {
        ...
        var allSets = ff.getPdfAnalyzers().stream()
            .map(analyzer -> ff.getFileProperties().entrySet().stream()
                .filter(e -> e.getValue().contains(analyzer))
                .map(Map.Entry::getKey)
                .collect(Collectors.toSet()))
            .filter(s -> !s.isEmpty())
            .toList();

        return allSets.isEmpty() || allSets.stream().allMatch(s -> s.equals(allSets.get(0)));
    }
}
