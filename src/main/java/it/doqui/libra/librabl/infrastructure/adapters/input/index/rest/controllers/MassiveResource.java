...
import it.doqui.libra.librabl.infrastructure.adapters.input.index.rest.components.impl.DtoMapper;
import it.doqui.libra.librabl.infrastructure.adapters.input.index.rest.components.interfaces.NodesBusinessInterface;
import it.doqui.libra.librabl.infrastructure.adapters.input.index.rest.dto.MassiveDeleteNodeRequest;
import it.doqui.libra.librabl.infrastructure.adapters.input.index.rest.dto.Node;
...
public class MassiveResource extends AbstractBridgeResource {
    ...
    public Response massiveCreateContent(@PathParam("tenantName") String tenant, @TraceParam(ignore = true) MultipartFormDataInput input) {
        return call(tenant, () -> {
            var parentNodeUidsString = getStringFromMultipart("parentNodeUids", input, true);
            var parentNodeUids = parentNodeUidsString.split("\\s*,\\s*");
            if (!(parentNodeUids.length > 0)) {
                throw new BadRequestException("parentNodeUids must contain at least one element");
            }

            var nodesRequests = getObjectFromMultipart("nodes", input, true, Node[].class);
