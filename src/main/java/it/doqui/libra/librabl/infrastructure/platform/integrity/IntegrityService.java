package it.doqui.libra.librabl.infrastructure.platform.integrity;
...
import it.doqui.libra.librabl.infrastructure.platform.events.SystemCheckEvent;
...
@ApplicationScoped
@Slf4j
public class IntegrityService implements IntegrityUseCase {
    ...
    void onStart(@Observes SystemCheckEvent ev) throws InterruptedException {
        ...
        if (StringUtils.isNotBlank(ev.getId())) {
            if (StringUtils.isBlank(ev.getCorrelationId())) {
                ...
                sendCheckMessage(UuidCreator.getTimeOrderedEpoch().toString(), ev.getId(), objectMapper.convertValue(info, Map.class));
            } else {
                // got response
    ...
    private SystemStatusInfo.InstanceInfo performLocalCheck() {
        ...
        instanceInfo.setName(transactionManagerPort.getInstanceId());
