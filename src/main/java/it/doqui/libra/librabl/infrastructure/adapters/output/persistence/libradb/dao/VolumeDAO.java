...
public class VolumeDAO extends AbstractDAO {
    ...
    private void calculateVolumes(Connection conn, Map<String, VolumeInfo> volumeMap) throws SQLException {
        ...
        try (var stmt = conn.prepareStatement(sql)) {
            try (var rs = stmt.executeQuery()) {
                while (rs.next()) {
                    var item = getVolumeInfo(rs.getString("tenant"), schema, volumeMap);
    ...
    private VolumeInfo getVolumeInfo(String tenant, String schema, Map<String, VolumeInfo> volumeMap) {
