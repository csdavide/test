...
import it.doqui.libra.librabl.infrastructure.adapters.input.index.cxf.mappers.AclMapper;
...
public class NodeServiceBridge extends AbstractServiceBridge {
    ...
    public TransactionInfo getTxnInfoFromDbid(long dbid, MtomOperationContext context) throws InvalidParameterException,
        ...
        return call(context, () -> nodeService
            .getNodeMetadata(new Vertex(VertexType.ID, dbid), Set.of(TX), null, Locale.getDefault())
            .map(n -> {
    ...
    public TransactionInfo getTxnInfoFromUID(Node node, MtomOperationContext context) throws InvalidParameterException,
        ...
        validate(node);
        return call(context, () -> nodeService
            .getNodeMetadata(new Vertex(VertexType.UUID, node.getUid()), Set.of(TX), null, Locale.getDefault())
            .map(n -> {
                var result = new TransactionInfo();
                result.setNodeId(n.getId());
                result.setUid(n.getUuid());
    ...
    private NodeInfoItem getNodeInfo(Node node, MtomOperationContext context) throws InvalidParameterException,
        ...
        validate(node);
        return call(context, () -> nodeService
                .getNodeMetadata(new Vertex(VertexType.UUID, node.getUid()), Set.of(NO_PROPERTIES), null, null))
                .map(n -> NodeInfoItem.builder()
                        .id(n.getId())
                        .tenant(n.getTenant())
                        .uuid(n.getUuid())
    ...
    public Content getContentMetadata(Node node, MtomOperationContext context)
        ...
        validate(node);
        return call(context, () -> nodeService
            .getNodeMetadata(new Vertex(VertexType.UUID, node.getUid()), Set.of(PARENT_ASSOCIATIONS, SYS_PROPERTIES, NO_NULL_PROPERTIES, LEGACY), null, Locale.getDefault())
    ...
    public Node[] massiveCreateContent(
        ...
        return call(context, () -> {
            try {
                List<String> uuids = nodeService.createNodes(inputs, Set.of());
    ...
    public Node createContentFromTemporaney(
        ...
        return call(context, () -> {
            ...
            ref.setTenant(temporaryUseCase.getTemporaryTenant());
    ...
    public void massiveDeleteNode(Node[] nodes, int mode, MtomOperationContext context)
        ...
        call(context, () -> {
            ...
            query.setUuids(Arrays.stream(nodes).map(Node::getUid).toList());
            var deleteJobRequest = new DeleteJobRequest();
            deleteJobRequest.setMode(OperationMode.SYNC);
            deleteJobRequest.setQuery(query);
            deleteJobRequest.setDeleteMode(deleteMode);
    ...
    public Node restoreContent(Node node, MtomOperationContext context)
        ...
        validate(node);
        return call(context, () -> {
            archiveService.restoreNode(new Vertex(VertexType.UUID, node.getUid()), null, LinkMode.ALL);
            return node;
        });
    }
    ...
    public Node copyNodeCopyChildren(Node source, Node parent, boolean copyChildren, Association newAssociation, MtomOperationContext context)
        ...
        return call(context, () -> {
            ...
            link.setParent(new Vertex(VertexType.UUID, parent.getUid()));
            if (newAssociation != null) {
                ...
                link.setType(newAssociation.getTypePrefixedName());
                link.setName(newAssociation.getPrefixedName());
            }
            link.setBindingType(BindingType.HARD);

            return new Node(nodeService.copyNode(new Vertex(VertexType.UUID, source.getUid()), link, copyChildren, true, CopyMode.NAME));
        });
    }
