...
import it.doqui.libra.librabl.infrastructure.adapters.output.persistence.libradb.entities.AccessRule;
...
import it.doqui.libra.librabl.infrastructure.platform.security.AuthorityUtils;
...
import it.doqui.libra.librabl.application.model.acl.InheritablePermissionItem;
...
import static it.doqui.libra.librabl.domain.model.session.UserContext.GROUP_EVERYONE;
...
public class AclDAO extends AbstractDAO {
    ...
    public SecurityGroup createUnmanagedSG(ApplicationTransaction tx, EditableSecurityGroup s) {
        ...
        sg.setTenant(sessionContext.getTenant());
        sg.setUuid(UuidCreator.getTimeOrderedEpoch().toString());
    ...
    public void createSecurityGroup(SecurityGroup sg) {
        call(conn -> {
            try {
                var sgSQL = """
                    insert into ecm_security_groups (tenant,uuid,name,tx,inheritance,managed,updated_at)\s
                    values (?,?,?,?,?,?,?)
                    """;
    ...
    public void updateInheritance(long sgId, boolean inheritance, long txId) {
        call(conn -> {
            var sql = """
                update ecm_security_groups set tx = ?, inheritance = ?, updated_at = ?\s
                where id = ?
                """;
            try (var stmt = conn.prepareStatement(sql)) {
                stmt.setLong(1, txId);
    ...
    public void renameSecurityGroupWhereName(String oldName, String newName) {
        call(conn -> {
            ...
            try (var stmt = conn.prepareStatement(sql)) {
                ...
                stmt.setString(2, sessionContext.getTenant());
