...
public class ModelResource extends AbstractResource {
    ...
    @Inject
    ModelManagerPort modelManagerPort;
        ...
        return call(() -> {
            ...
            modelManagerPort
                ...
                .forEach(model -> {
                    ...
                    if (flat) {
                        result.getAspects().addAll(model.getAspects().stream().map(a -> modelManagerPort.getFlatAspect(a.getName())).toList());
                        result.getTypes().addAll(model.getTypes().stream().map(t -> modelManagerPort.getFlatType(t.getName())).toList());
                    } else {
    ...
    public Response getModel(
        ...
        return call(() -> {
            var model = exact
                ? modelManagerPort.getModel(name)
                : modelManagerPort.getNamespaceSchema(name);
    ...
    public Response getInternalModel(
        @PathParam("name") String name) {
        return call(() -> modelManagerPort.getModel(name)
            .map(model -> {
                try {
    ...
    public Response getStoredModel(
        @PathParam("name") String name) {
        return call(() -> modelManagerPort.getStoredModel(name)
    ...
    public Response deployModel(
        ...
        return call(() -> {
            ...
            modelManagerPort.deployModel(fmt, is);
    ...
    public Response undeployModel(@PathParam("name") String modelName) {
        return call(() -> {
            modelManagerPort.undeployModel(modelName);
            return Response.noContent().build();
        });
    }
}
