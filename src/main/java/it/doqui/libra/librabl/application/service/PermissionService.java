...
import it.doqui.libra.librabl.application.ports.in.PermissionUseCase;
...
public class PermissionService implements PermissionUseCase {
    ...
    public void removeAllAuthorityPermissions(String uuid, String authority) {
        txManager.perform(tx -> {
            var node = findManagedNode(uuid);
            permissionValidator.requirePermission(node, PermissionFlag.A);
            var a = AuthorityUtils.normalizeAuthority(authority, sessionContext.getUserContext().getTenantRef());
    ...
    private void alterInheritanceOnPaths(ApplicationTransaction tx, ActiveNode node) {
        ...
        if (inheritance) {
            transactionManagerPort.connection(conn -> {
               var sql = """
                   select\s
                     c.node_path,
                   ...
                   from ecm_paths c\s
                   join ecm_paths p on (p.node_id = c.parent_id and c.node_path like concat(p.node_path, '%'))\s
                   where c.node_id = ?
                   """;
