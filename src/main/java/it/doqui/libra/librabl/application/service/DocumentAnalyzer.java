...
import it.doqui.libra.librabl.infrastructure.adapters.output.droid.DroidAnalyzer;
...
import static it.doqui.libra.librabl.application.model.document.PDFAnalyzer.*;
import static it.doqui.libra.librabl.domain.model.graph.Constants.*;

@ApplicationScoped
@Slf4j
public class DocumentAnalyzer implements DocumentAnalyzeUseCase {
    ...
    @ConfigProperty(name = "libra.formats.tika.timeout", defaultValue = "PT15S")
    Duration tikaTimeout;

    @ConfigProperty(name = "libra.analyzer.core-pool-size", defaultValue = "4")
    int corePoolSize;

    @ConfigProperty(name = "libra.analyzer.max-pool-size", defaultValue = "20")
    int maxPoolSize;

    @ConfigProperty(name = "libra.analyzer.queue-capacity", defaultValue = "100")
    int queueCapacity;

    @ConfigProperty(name = "libra.analyzer.keep-alive-time", defaultValue = "PT60S")
    ...
    void init() {
        executorService = new ThreadPoolExecutor(
            ...
            new ThreadFactoryBuilder()
                .setNameFormat("document-analyzer-%d")
                .setDaemon(true)
                .build(),
    ...
    public FileFormatDescriptor getFileFormat(File file, String fileName) throws AnalysisException {
        ...
        try (var fis = new FileInputStream(file)) {
            var fileCharacteristics = getFileAndSignatureType(fis, isPdf);
    ...
    private FileCharacteristics getFileAndSignatureType(InputStream stream, Boolean isPdf) throws AnalysisException {
        ...
        try {
            ...
            try (var bais = new ByteArrayInputStream(data)) {
                ...
                if (signature != null && Strings.CS.equals(signature.getNamespaceURI(), "http://www.w3.org/2000/09/xmldsig#")) {
            ...
            try {
                new CMSSignedData(decodedCMS(data));
    ...
    private void analyzeWithPdfBox(byte[] data, Boolean isPdf, FileCharacteristics result) {
        try {
            final var parser = new PDFParser(new RandomAccessReadBuffer(data));
            try (final var pdDocument = parser.parse(true)) {
    ...
    private void manageExtensions(List<FileFormatDescriptor.FileFormatItem> items, String fileName) {
        ...
        items.forEach(ffi -> {
            if (ffi.getExtensions() != null && ffi.getExtensions().size() > 1) {
                var matchingExtension = ffi.getExtensions().stream().filter(ext -> Strings.CS.equals(ext, fileName.substring(fileName.lastIndexOf('.') + 1))).findFirst().orElse(null);
                var otherExtensions = ffi.getExtensions().stream().filter(ext -> !Strings.CS.equals(ext, fileName.substring(fileName.lastIndexOf('.') + 1))).toList();
    ...
    private boolean isPdfFile(List<String> extensions, List<String> mimetypes, String fileName) {
        ...
        if (extensions.contains(EXT_PDF) || mimetypes.contains(MIMETYPE_PDF)) {
            return true;
        }
        return fileName != null && fileName.endsWith("." + EXT_PDF);
    }
    ...
    private void addLibraRecognizing(FileFormatDescriptor ffd, File file) {
        ...
        var droidExt = ffd.getItems().stream()
            .filter(ffi -> Objects.equals(ffi.getFormatIdentifier(), FileFormatDescriptor.FormatIdentifier.DROID))
            .flatMap(ffi -> ffi.getExtensions().stream())
            .findFirst();

        if (tikaMimetypes.size() == 1
            && tikaMimetypes.contains(MIMETYPE_TEXT_PLAIN) && (Objects.equals(ffd.getFileType(), FileType.XML) || droidMimetypes.contains(MIMETYPE_APP_XML) || droidMimetypes.contains(MIMETYPE_TEXT_XML))) {
            ffd.getItems().add(createLibraRecognizingItem(EXT_XML));
        }

        try (var workbook = WorkbookFactory.create(file)) {
            workbook.isHidden();
        } catch (EncryptedDocumentException e) {
            if (droidPuids.size() == 1 && droidPuids.contains("fmt/494") && droidMimetypes.isEmpty() && droidExt.isPresent()) {
                ffd.getItems().add(createLibraRecognizingItem(droidExt.get()));
            }
        } catch (Exception e) {
            if (e.getCause() == null) {
        ...
        //TODO: mancano i rar [issue 21]
    }

    private FileFormatDescriptor.FileFormatItem createLibraRecognizingItem(String extension) {
        log.debug("Adding LIBRA recognizing...");
        var ffi = new FileFormatDescriptor.FileFormatItem();
        ffi.setFormatIdentifier(FileFormatDescriptor.FormatIdentifier.LIBRA);
        ffi.getMimeTypes().add(getMimetype(extension));
        ffi.setResult(FileFormatDescriptor.Result.POSITIVE_GENERIC);
    ...
    private String getMimetype(String ext) {
        switch (ext) {
            case EXT_XML -> {return MIMETYPE_APP_XML;}
            case EXT_DOCX -> {return MIMETYPE_DOCX_PROTECTED;}
            case EXT_XLSX -> {return MIMETYPE_XLSX_PROTECTED;}
            case EXT_PPTX -> {return MIMETYPE_PPTX_PROTECTED;}
            default -> {return null;}
        }
    }

    private byte[] decodedCMS(byte[] input) {
        // Nessuna corversione
        if (input[0] == 0x30) return input;

        // Base64
        String inputString = new String(input);
        if (inputString.isBlank()) return input;

        if (inputString.matches("^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{4})$")) {
            log.debug("Decoding Base64...");
            return Base64.getDecoder().decode(inputString.trim());
        }

        // PEM (con headers)

        String derStr = inputString.replaceAll("-----BEGIN (?:PKCS7|CMS).*?-----\\n?", "")
            .replaceAll("\\n?-----END (?:PKCS7|CMS).*?-----", "")
            .replaceAll("\\s+", "");
        if (!derStr.equals(inputString)) {
            log.debug("Extracting PEM, then converting into Base64 then decoding it...");
            return Base64.getDecoder().decode(derStr.trim());
        }
